...
const cacheRoot = join(serverDir, 'music', 'cache');
...
export class MusicCache {
  ...
  async getArtwork(sourceUrl) {
    const allowedHosts = new Set(['yt3.googleusercontent.com', 'i.ytimg.com', 'lh3.googleusercontent.com']);
    const parsed = new URL(sourceUrl);
    if (!allowedHosts.has(parsed.hostname)) throw new Error('Artwork host is not allowed');
